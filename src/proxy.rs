...
async fn main() {
    ...
    // ROUTES
    let app = Router::new()
        .nest("/json", json::router(&pool))
        .nest("/mpack", mpack::router(&pool));
...
mod mpack {
    ...
    async fn create_new_benchmark(
        ...
        MsgPack(body): MsgPack<BenchmarkConfiguration>,
    ) -> Result<MsgPack<Benchmark>, (StatusCode, String)> {
        let r = pool
            .get()
            .await
            .unwrap()
            ...
            .await
            .unwrap();
        Ok(MsgPack(r.into_inner()))
    }
    
    async fn start_benchmark(
        State(pool): State<Pool>,
        MsgPack(body): MsgPack<Benchmark>,
    ...
    async fn next_batch(
        State(pool): State<Pool>,
        MsgPack(body): MsgPack<Benchmark>,
    ) -> Result<MsgPack<Batch>, (StatusCode, String)> {
